...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
    uint8_t rawData[14];  // Register data (6 bytes for accel, 6 bytes for gyro)

    // Read 14 bytes of data from the ACCEL_XOUT_H register (6 bytes accel + 6 bytes gyro)
    readByte(ACCEL_XOUT_H, 14, rawData);  // Assuming `readByte` works as intended

    // Convert the 8-bit values into 16-bit values (high byte << 8 | low byte)
    int16_t ax_raw = (int16_t)((rawData[0] << 8) | rawData[1]);
    int16_t ay_raw = (int16_t)((rawData[2] << 8) | rawData[3]);
    int16_t az_raw = (int16_t)((rawData[4] << 8) | rawData[5]);
    int16_t gx_raw = (int16_t)((rawData[8] << 8) | rawData[9]);
    int16_t gy_raw = (int16_t)((rawData[10] << 8) | rawData[11]);
    int16_t gz_raw = (int16_t)((rawData[12] << 8) | rawData[13]);

    // Convert raw accelerometer data to g (assuming ±2g range, scaling factor 16384)
    // This factor is specific to the accelerometer sensitivity setting
    // For ±2g, sensitivity is 32768 counts per g
    *ax = (float)ax_raw * aRes;
    *ay = (float)ay_raw * aRes;
    *az = (float)az_raw * aRes;

    // Convert raw gyroscope data to degrees per second (assuming ±250°/s range, scaling factor 131)
    // This factor is specific to the gyroscope sensitivity setting
    // For ±250°/s, sensitivity is 32768 counts per 250°/s
    *gx = (float)gx_raw * gRes;
    *gy = (float)gy_raw * gRes;
    *gz = (float)gz_raw * gRes;

    // Optional: If you have biases, subtract them (accelerometer/gyro biases need to be calculated beforehand)
    *ax -= accelBias[0];
    *ay -= accelBias[1];
    *az -= accelBias[2];
    *gx -= gyroBias[0];
